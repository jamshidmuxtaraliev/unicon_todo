...
//
// const Duration kInterval = Duration(minutes: 1);
//
...
//     try {
//       // Bir marta init kifoya, lekin qayta chaqirilsa ham xato bermaydi
...
//       );
//     } catch (e) {
//       if (kDebugMode) print('WorkManager init/register error: $e');
//     }
//
//     // 2) Flutter background service INIT
...
//     );
//
//     // 3) Har chaqirilganda ehtiyot uchun startService()
//     await service.startService();
//
//     // 4) Darhol reschedule signalini yuboramiz
//     refresh();
//   }
//
...
//   registerOnMarkDone((int id) async {
//     await ds.toggleTask(id, true);            // DB: done = 1
//     _timers.remove(id)?.cancel();             // agar timer bor bo'lsa to'xtat
//
//     final t = await ds.getTaskById(id);
//     if (t != null) {
//       await showTaskDoneNotification(taskId: id, title: t.title); // "Bajarildi" xabari
//     }
//
//     // HOME WIDGET hisoblarini yangilash
...
//     } catch (_) {
//       // Agar background izolatda bevosita bo'lmasa, UI izolatga signal yuboramiz
//       FlutterBackgroundService().invoke('widget_refresh', {
//         'all': all, 'done': done, 'undone': undone,
//       });
//     }
//   });
//
...
//
//   // Servis ishga tushganda bir marta hammasini qayta rejalashtiramiz
//   await _rescheduleAll(ds);
//
//   // Drift/Fallback mini-tiklash: 1 daqiqada bir tekshirish
...
//   try {
//     // 1) Kechikkan (due) tasklarni darhol chiqarish va +1h re-arm
//     final due = await ds.getDueTasksForNotify(interval: kInterval);
//     for (final t in due) {
//       if (t.id == null || t.done == true) continue;
//       await showTaskNotification(taskId: t.id!, title: t.title);
...
//         // Hali 1 soat to‘lmadi
//         remain = firstFire.difference(now);
//       } else {
//         // 1 soat allaqachon o‘tgan — “grid” bo‘yicha keyingi slotga yo‘naltiramiz
...
//     }
//
//     await showTaskNotification(taskId: id, title: task.title);
...
Future<bool> onStart(ServiceInstance service) async {
  ...
  if (service is AndroidServiceInstance) {
    ...
    service.setForegroundNotificationInfo(
      title: 'Task Manager',
      content: 'Eslatmalar xizmati ishlamoqda…',
    );
  }
